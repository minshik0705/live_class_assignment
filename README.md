...
```mermaid
flowchart LR
    G["이벤트 생성기<br/>(Python, 백필 시뮬레이터)"] -->|"batch INSERT"| P["PostgreSQL 16<br/>일 단위 파티셔닝 + BRIN"]
    P -->|"파티션 단위 내보내기"| A["Parquet + zstd<br/>(장기 보관 아카이브)"]
    P -->|"SQL"| GF["Grafana<br/>(파일 프로비저닝 대시보드)"]
```
...
2. **generator** 컨테이너가 실행 시점 기준 **과거 7일치 이벤트 약 5.5만 건**을 시뮬레이션해 일별 파티션에 적재 (1분 내외 소요)
...
생성량·기간은 `docker-compose.yml`의 `GEN_*` 환경변수로 조절할 수 있고, 시드 고정(`GEN_SEED`)으로 매 실행 동일한 데이터가 재현됩니다.
...
이벤트 타입은 임의로 고르지 않고, **라이브 강의 플랫폼이 크리에이터에게 실제로 보여주는 대시보드 지표(방문자 수, 가입자 수, 수강신청 수, 거래액 결제/취소)를 역산**해 그 지표를 재현할 수 있는 최소 집합으로 정했습니다. "어떤 분석을 할 것인가"가 먼저고, 이벤트는 그 분석의 원천 데이터입니다.
...
독립 랜덤 추출은 "구매했는데 방문한 적 없는 유저" 같은 논리적으로 모순된 로그를 만듭니다. 분석할 가치가 있는 데이터가 되도록 5가지를 넣었습니다.
...
4. **에러율** — 트래픽에 비례한 ~1.5%
5. **무료/유료 분기 + 환불** — 무료 클래스는 enroll만, 유료는 enroll+purchase. 환불은 결제의 ~7%가 1~72시간 뒤 발생. **전액 환불만 구현** — 부분 환불·환불 사유는 이 과제 스코프에서 의도적으로 제외했습니다 (현실성 대비 복잡도가 수지에 안 맞는 지점이라 판단)

### 생성 시각: 백필 시뮬레이터

생성기는 "현재 시각 기록기"가 아니라 **과거 7일치 역사의 시뮬레이터**입니다. 현재 시각만 찍으면 모든 데이터가 당일 파티션 하나에 몰려, 일 단위 파티셔닝도 7일 추이 차트도 의미가 없어집니다. 실행 시점 기준 `[now−7d, now]` **상대 윈도우**에 event_time을 배정하므로, 평가자가 언제 `docker compose up`을 해도 Grafana의 "최근 7일" 기본 시간창에 데이터가 보입니다.
...
- **단일 와이드 팩트 테이블 + 차원 1개**: 분석 쿼리(타입별 카운트, 시간대 추이, 유저별 합계)가 전부 이벤트 타입을 **가로지르기** 때문에, 타입별 테이블 분리 대신 한 테이블에 두고 `GROUP BY event_type` 한 줄로 끝나게 했습니다. 읽기 패턴이 스키마를 결정합니다.
- **타입 전용 컬럼은 nullable**: `amount`는 purchase/refund만, `error_code`는 error만 값을 가집니다. PG는 행마다 컬럼당 1비트 null bitmap만 쓰고 NULL 값은 공간을 차지하지 않으므로, 와이드 테이블의 NULL은 사실상 공짜입니다.
...
| MySQL | 가능하지만 채택 근거가 "익숙함"뿐. 시계열 물리 설계 수단(BRIN 등)이 PG보다 빈약 |
| DuckDB | 집계 성능은 최고지만 임베디드라 "앱+DB 컨테이너 구성"의 그림이 어색하고 BI 연결·동시 쓰기 제약 |
| SQLite | 파티셔닝·동시성·스케일 서사 부재 |
| MongoDB | 과제의 "JSON 통째 저장 금지" 요건과 문서 모델이 정면 충돌 |
...
"이론상 유리"가 아니라 실측으로 증명합니다 — 같은 데이터의 PG 파티션 크기(인덱스 포함) vs Parquet 파일 크기가 `parquet_output/size_report.txt`로 자동 생성됩니다 (아래 "고민한 점"에 실측 수치).
...
**Grafana + 파일 기반 프로비저닝**. datasource YAML과 대시보드 JSON을 repo에 커밋해 두어, 컨테이너 기동만으로 PG 연결과 대시보드가 자동 구성됩니다. 익명 조회 + 홈 대시보드 지정으로 **`docker compose up` 후 localhost:3000을 열면 클릭 0회로 데이터가 채워진 대시보드**가 보입니다.

비즈니스 지표 성격상 Metabase도 적합하지만, 초기 설정이 UI 클릭에 의존해 자동화하려면 비공식 API 스크립트가 필요합니다. **파일 프로비저닝이 공식 기능이어서 재현성을 보장할 수 있는 Grafana**를 선택했습니다 — "남이 한 번에 실행되는 경험"을 시각화까지 일관되게 가져가기 위한 결정입니다.

<!-- 대시보드 스크린샷: docs/dashboard.png 추가 후 아래 주석 해제 -->
...
처음 설계에서 가장 먼저 잡은 문제. 생성기가 현재 시각을 찍으면 일 단위 파티셔닝·7일 추이·아카이브 시연이 전부 무의미해집니다. "생성기는 기록기가 아니라 과거 N일치 역사의 시뮬레이터다"로 관점을 바꾸고, 절대 날짜가 아닌 **상대 윈도우**(실행 시점 기준 과거 7일)로 구현해 평가 시점과 무관하게 재현되도록 했습니다.

### ② properties JSONB 컬럼 — 알지만 배제

타입별 가변 필드를 JSONB 하나에 넣는 설계는 실무에서 흔하고(스키마 변경 없이 타입 추가) 고려했지만 배제했습니다. ① 과제의 "JSON 통째 저장 금지" 요건의 회색지대이고, 출제 의도(효율적 저장에 대한 고민)를 정면으로 충족하는 쪽을 택했습니다. ② JSON 내부 값(amount) 집계는 매번 파싱이 필요하고 컬럼 통계·인덱스 활용이 어렵습니다.

### ③ Skew를 어디서 만들고 어디서 무해화하는가

- **생성**: 일부러 만든다 (Zipf 2축 + 시간대 피크) — 현실 데이터의 특성이므로
- **저장**: 컬럼 저장에서는 오히려 호재 — 반복 값일수록 dictionary encoding 효율 상승 (Parquet 트랙의 근거)
- **파티션**: 파티션 키가 user_id가 아닌 **시간**이라 엔티티 skew가 파티션 skew로 전이되지 않습니다 (user_id 해시 파티셔닝이었다면 핫 파티션 발생). 시간대 피크는 **일 단위** 파티션이 하루 안에서 흡수합니다 — hour 단위였으면 저녁 파티션만 비대해졌을 것이므로, "왜 일 단위인가"의 근거이기도 합니다
- 헤비유저 조회는 B-tree가 정상 처리하고, skew가 진짜 문제가 되는 건 분산 처리 스케일(핫 파티션·스트래글러)입니다 — 아래 스케일 시나리오
...
"컬럼 저장이 유리하다"를 이론으로 두지 않고 같은 데이터로 실측했습니다 (실행 시 `parquet_output/size_report.txt`로 자동 생성, 아래는 본 환경 실측치).

| | PG 파티션 합계 (인덱스 포함) | Parquet+zstd 합계 | 비율 |
|---|---|---|---|
| 7일치 54,756건 | 8.5 MB | 1.7 MB | **4.9x** |
...
- **S3** — Parquet 아카이브의 목적지인 **콜드 저장소**. 저장 단가가 DB 스토리지보다 압도적으로 낮고 내구성이 높아 "오래된 파티션 내보내기" 경로의 종착지로 자연스럽습니다. 내보내기는 Lambda 또는 ECS 스케줄 태스크로 일 1회 배치.
- **Athena** — S3의 Parquet을 **이동 없이 직접 쿼리**. 별도 클러스터를 상시로 띄우지 않고 쿼리당 과금이라, "가끔 보는 과거 데이터"라는 접근 패턴에 비용 구조가 맞습니다. 컬럼 저장(Parquet)의 집계 이점을 그대로 활용합니다.
...
수집 계층(생성기 → DB) 사이에 Kinesis 같은 스트리밍 버퍼를 둘지 가장 오래 고민했습니다. 장점은 명확합니다 — DB 장애 시 이벤트 유실 방지(버퍼링), 쓰기 피크 흡수, 컨슈머 확장. 그러나 현재 스케일(일 수만 건)에서는 RDS 직접 INSERT로 충분하고, Kinesis는 샤드 관리·컨슈머 운영이라는 새 복잡도를 가져옵니다. **"언젠가 필요할 것"으로 지금 복잡도를 사지 않는다**를 원칙으로 배제하되, 트래픽 100배 시점에 수집 계층만 교체하면 되도록 생성기-저장소 사이를 단일 진입점으로 유지했습니다. 구성도에서 Kinesis가 들어갈 자리는 ECS와 RDS 사이입니다.
